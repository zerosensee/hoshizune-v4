...
/**
 * Серверный каталог тарифов подписок.
 * Клиент передаёт ТОЛЬКО planId, durationDays вычисляется исключительно на сервере.
 */
...
export function createSubscription({
  ...
  if (days > 3650) days = 3650; // Жесткий предел длительности
  const expiresAt = now + days * 86400 * 1000;

  // Деактивируем старые активные подписки данного пользователя
  try {
    db.prepare("UPDATE subscriptions SET status = 'replaced' WHERE user_id = ? AND status = 'active'").run(userId);
  } catch {}
...
export function getAllSubscriptions() {
  ...
  try {
    ...
    const rows = db.prepare(`
      SELECT p.id as profile_id, p.user_id, p.display_name, p.slug,
      ...
      LEFT JOIN user_emails ue ON (ue.user_id = p.user_id OR ue.user_id = u.id) AND ue.is_primary = 1
      LEFT JOIN subscriptions s ON (s.user_id = p.user_id OR s.user_id = p.id OR s.user_id = u.id OR (s.email IS NOT NULL AND s.email != '' AND LOWER(s.email) = LOWER(ue.email))) AND s.status = 'active'
