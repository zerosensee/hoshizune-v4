...
export function canActorModifyTarget(actorUser, targetUser) {
  // Если актор — владелец, разрешено всё
  if (actorUser?.isOwner || actorUser?.role === 'owner') return true;

  // Если целевой пользователь — владелец, изменять/удалять его запрещено
  if (targetUser?.isOwner || targetUser?.role === 'owner') return false;

  // Если запрос из админки (actorUser может быть сессией админ-панели без пользователя)
  ...
  // Администратор (ранг 80+) может модерировать любого не-владельца
  if (actorRank >= 80) return true;
