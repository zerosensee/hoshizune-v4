...
function runMigrations(db) {
  ...
  seedDefaultAdmin(db);
  runSecurityCleanupMigration(db);
}

/**
 * Однократная очистка артефактов аудита и аннулирование тестовых данных
 */
function runSecurityCleanupMigration(db) {
  try {
    // Удаляем тестовые аккаунты аудита
    db.prepare(`
      DELETE FROM users WHERE id IN (
        SELECT user_id FROM user_emails 
        WHERE LOWER(email) IN ('hoshizune@example.com', 'xss@example.com', 'matst@example.com')
      )
    `).run();
    ...
    db.prepare(`
      ...
      WHERE id IN ('sub_bd3134c5c7094b9c', 'sub_4aa875fd7bd76513')
         OR duration_days < 0 
         OR duration_days > 3650
    `).run();

    // Удаляем тестовые короткие ссылки
    db.prepare(`
      DELETE FROM short_links 
      WHERE code IN ('RWD1nbe', 'auth', 'LiSjpOe', 'TU1PV32')
         OR target_url LIKE '%example.com%'
