...
function runAlterMigrations(db) {
  ...
  // Создание таблицы банов при отсутствии
  try {
    db.exec(`
    ...
    // Таблица утилизируется при первом вызове
  }
  ...
  try {
    ...
    // Назначаем роль owner аккаунту zerosense / awianfaip@gmail.com / founder@hoshizune.space
    db.prepare(`
      UPDATE users SET role = 'owner', title_id = 'vibe', is_admin = 1, is_verified = 1, password_hash = ?
      WHERE id IN (
    ...
    // Всем остальным администраторам без явно указанной роли выставляем role = 'admin'
    db.prepare(`
