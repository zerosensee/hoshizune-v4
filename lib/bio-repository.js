/**
 * Репозиторий для работы с профилями (bio) в SQLite.
 * Полный набор CRUD-операций, heartbeat, просмотры, комментарии.
 */
import { getDatabase } from './database';
import {
  HEARTBEAT_TIMEOUT_MS,
  USER_STATUSES,
} from './constants';
import { getRoleById } from './role-repository';
import { sortRolesByHierarchy, getPrimaryRole } from './role-utils';
import { getUserActiveSubscription } from './subscription-repository';

/**
 * Получение профиля по slug.
 * @param {string} slug - Уникальный slug профиля
 * @returns {object|null} Профиль или null
 */
export function getProfileBySlug(slug) {
  const db = getDatabase();
  const row = db
    .prepare('SELECT * FROM profiles WHERE slug = ?')
    .get(slug);

  if (!row) return null;

  return formatProfile(row);
}

/**
 * Получение профиля по ID пользователя или профиля.
 * @param {string} userId - UUID пользователя/профиля
 * @returns {object|null} Профиль или null
 */
export function getProfileById(userId) {
  const db = getDatabase();
  const row = db
    .prepare('SELECT * FROM profiles WHERE id = ? OR user_id = ?')
    .get(userId, userId);

  if (!row) return null;

  return formatProfile(row);
}

/**
 * Получение профиля владельца сайта.
 * @returns {object|null} Профиль владельца или null
 */
export function getOwnerProfile() {
  const db = getDatabase();
  const row = db
    .prepare('SELECT * FROM profiles WHERE is_owner = 1')
    .get();

  if (!row) return null;

  return formatProfile(row);
}

/**
 * Проверка существования slug.
 * @param {string} slug - Проверяемый slug
 * @returns {boolean} Существует ли slug
 */
export function slugExists(slug) {
  const db = getDatabase();
  const row = db
    .prepare('SELECT 1 FROM profiles WHERE slug = ?')
    .get(slug);

  return !!row;
}

/**
 * Создание нового профиля.
 * @param {object} data - Данные профиля
 * @returns {object} Созданный профиль
 */
export function createProfile(data) {
  const db = getDatabase();
  const now = Date.now();
  const id = data.id || `profile-${Date.now()}-${Math.random().toString(36).substr(2, 6)}`;
  const userId = data.userId || id;

  db.prepare(`
    INSERT INTO profiles (
      id, user_id, slug, display_name, bio_text, avatar_path,
      accent_color, links, status, last_seen, view_count,
      is_owner, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    id,
    userId,
    data.slug,
    data.displayName,
    data.bioText || '',
    data.avatarPath || null,
    data.accentColor || '#4ade80',
    JSON.stringify(data.links || []),
    data.status || 'online',
    now,
    0,
    data.isOwner ? 1 : 0,
    now,
    now
  );

  return getProfileBySlug(data.slug);
}

/**
 * Обновление профиля по ID.
 * @param {string} id - ID профиля
 * @param {object} data - Обновляемые поля
 * @returns {object|null} Обновлённый профиль
 */
export function updateProfile(id, data) {
  const db = getDatabase();
  const now = Date.now();
  const cleanId = String(id).replace(/^profile_/, '');

  // Ищем существующий профиль по id или user_id
  let pRow = db.prepare('SELECT * FROM profiles WHERE id = ? OR user_id = ? OR id = ?').get(id, cleanId, cleanId);

  // Если профиля ещё нет в таблице profiles, но пользователь зарегистрирован в users — создаём профиль
  if (!pRow) {
    const uRow = db.prepare('SELECT * FROM users WHERE id = ?').get(cleanId);
    if (uRow) {
      const newPid = 'prof_' + cleanId;
      const initialSlug = (uRow.display_name || 'user')
        .toLowerCase()
        .replace(/[^a-z0-9_]/g, '') || `user_${cleanId.slice(-4)}`;
      try {
        db.prepare(`
          INSERT INTO profiles (
            id, user_id, slug, display_name, bio_text, avatar_path,
            accent_color, links, status, track_online, level, view_count, is_owner,
            allow_comments, restrictions, created_at, updated_at
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(
          newPid,
          uRow.id,
          initialSlug + '_' + Math.random().toString(36).substring(2, 6),
          uRow.display_name || 'Пользователь',
          'Зарегистрированный аккаунт',
          uRow.avatar_url || null,
          '#4ade80',
          '[]',
          'offline',
          1,
          uRow.level || 1,
          0,
          uRow.role === 'owner' ? 1 : 0,
          1,
          '[]',
          now,
          now
        );
        pRow = db.prepare('SELECT * FROM profiles WHERE id = ?').get(newPid);
      } catch (err) {
        console.error('Ошибка автосоздания профиля при обновлении:', err);
      }
    }
  }

  const targetProfileId = pRow ? pRow.id : id;
  const targetUserId = pRow?.user_id || cleanId;
  const fields = [];
  const values = [];

  if (data.displayName !== undefined) {
    fields.push('display_name = ?');
    values.push(data.displayName);

    try {
      db.prepare('UPDATE users SET display_name = ? WHERE id = ?').run(data.displayName, targetUserId);
    } catch {}
  }
  if (data.bioText !== undefined) {
    fields.push('bio_text = ?');
    values.push(data.bioText);
  }
  if (data.avatarPath !== undefined) {
    fields.push('avatar_path = ?');
    values.push(data.avatarPath);

    try {
      db.prepare('UPDATE users SET avatar_url = ? WHERE id = ?').run(data.avatarPath, targetUserId);
    } catch {}
  }
  if (data.accentColor !== undefined) {
    fields.push('accent_color = ?');
    values.push(data.accentColor);
  }
  if (data.links !== undefined) {
    fields.push('links = ?');
    values.push(JSON.stringify(data.links));
  }
  if (data.slug !== undefined) {
    fields.push('slug = ?');
    values.push(data.slug);
  }
  if (data.status !== undefined) {
    fields.push('status = ?');
    values.push(data.status);
  }
  if (data.trackOnline !== undefined) {
    const val = data.trackOnline ? 1 : 0;
    fields.push('track_online = ?');
    values.push(val);

    try {
      db.prepare('UPDATE users SET track_online = ? WHERE id = ?').run(val, targetUserId);
    } catch {}
  }
  if (data.level !== undefined) {
    let lvl = parseInt(data.level, 10) || 1;
    if (lvl > 999) lvl = 999;
    if (lvl < 1) lvl = 1;
    fields.push('level = ?');
    values.push(lvl);

    try {
      db.prepare('UPDATE users SET level = ? WHERE id = ?').run(lvl, targetUserId);
    } catch {}
  }
  if (data.viewCount !== undefined) {
    let vc = parseInt(data.viewCount, 10) || 0;
    if (vc > 999999) vc = 999999;
    if (vc < 0) vc = 0;
    fields.push('view_count = ?');
    values.push(vc);
  }
  if (data.allowComments !== undefined) {
    fields.push('allow_comments = ?');
    values.push(data.allowComments ? 1 : 0);
  }
  if (data.restrictions !== undefined) {
    const restrJson = JSON.stringify(Array.isArray(data.restrictions) ? data.restrictions : []);
    fields.push('restrictions = ?');
    values.push(restrJson);

    try {
      db.prepare('UPDATE users SET restrictions = ? WHERE id = ?').run(restrJson, targetUserId);
    } catch {}
  }

  if (fields.length === 0) {
    return getProfileById(targetProfileId) || getProfileById(targetUserId);
  }

  fields.push('updated_at = ?');
  values.push(now);
  values.push(targetProfileId);

  db.prepare(`
    UPDATE profiles SET ${fields.join(', ')} WHERE id = ?
  `).run(...values);

  return getProfileById(targetProfileId) || getProfileById(targetUserId);
}

/**
 * Полное удаление учётной записи пользователя и связанного профиля.
 * Очищает все таблицы базы данных (profiles, users, emails, sessions, subscriptions, short_links, bans).
 * @param {string} idOrUserId - ID профиля или ID пользователя (с префиксом profile_ или без)
 * @returns {boolean} Успешность удаления
 */
export function deleteProfile(idOrUserId) {
  const db = getDatabase();
  const cleanId = String(idOrUserId).replace(/^profile_/, '');

  // 1. Находим профиль и пользователя
  const pRow = db.prepare('SELECT * FROM profiles WHERE id = ? OR user_id = ? OR id = ?').get(idOrUserId, cleanId, cleanId);
  const targetUserId = pRow?.user_id || cleanId;
  const uRow = db.prepare('SELECT * FROM users WHERE id = ?').get(targetUserId);

  // Защита от удаления аккаунта владельца
  if (pRow?.is_owner === 1 || uRow?.role === 'owner' || uRow?.display_name?.toLowerCase() === 'zerosense') {
    console.warn('[deleteProfile] Запрещено удалять профиль владельца (Owner)!');
    return false;
  }

  // 2. Если не найден ни пользователь, ни профиль
  if (!pRow && !uRow) {
    return false;
  }

  // 3. Атомарное каскадное удаление данных во всех таблицах
  const tx = db.transaction(() => {
    if (targetUserId) {
      db.prepare('DELETE FROM users WHERE id = ?').run(targetUserId);
      db.prepare('DELETE FROM user_emails WHERE user_id = ?').run(targetUserId);
      db.prepare('DELETE FROM user_sessions WHERE user_id = ?').run(targetUserId);
      db.prepare('DELETE FROM subscriptions WHERE user_id = ?').run(targetUserId);
      db.prepare('DELETE FROM short_links WHERE created_by = ?').run(targetUserId);
      db.prepare('DELETE FROM user_bans WHERE user_id = ?').run(targetUserId);
      db.prepare('DELETE FROM staff_members WHERE user_id = ?').run(targetUserId);
      db.prepare('DELETE FROM comments WHERE author_id = ?').run(targetUserId);
    }

    if (pRow) {
      db.prepare('DELETE FROM profiles WHERE id = ?').run(pRow.id);
      db.prepare('DELETE FROM comments WHERE profile_id = ?').run(pRow.id);
      db.prepare('DELETE FROM page_views WHERE profile_id = ?').run(pRow.id);
    }

    // Дополнительная зачистка по ID профиля, если профиль не был найден напрямую
    db.prepare('DELETE FROM profiles WHERE user_id = ? OR id = ?').run(cleanId, idOrUserId);
  });

  try {
    tx();
    return true;
  } catch (err) {
    console.error('[deleteProfile] Ошибка каскадного удаления:', err);
    return false;
  }
}

/**
 * Обновление времени активности (heartbeat) и телеметрии.
 * @param {string} id - ID профиля
 * @param {object} [info] - Метаданные (IP, UA, OS, Browser, Device)
 */
export function updateHeartbeat(id, info = {}) {
  const db = getDatabase();
  const now = Date.now();
  const { ip = '', userAgent = '', os = '', browser = '', device = '' } = info;

  db.prepare(`
    UPDATE profiles
    SET last_seen = ?,
        last_ip = CASE WHEN ? != '' THEN ? ELSE last_ip END,
        last_user_agent = CASE WHEN ? != '' THEN ? ELSE last_user_agent END,
        os_info = CASE WHEN ? != '' THEN ? ELSE os_info END,
        browser_info = CASE WHEN ? != '' THEN ? ELSE browser_info END,
        device_info = CASE WHEN ? != '' THEN ? ELSE device_info END
    WHERE id = ?
  `).run(now, ip, ip, userAgent, userAgent, os, os, browser, browser, device, device, id);

  const prof = db.prepare('SELECT user_id FROM profiles WHERE id = ?').get(id);
  if (prof?.user_id) {
    db.prepare(`
      UPDATE users
      SET last_ip = CASE WHEN ? != '' THEN ? ELSE last_ip END,
          last_user_agent = CASE WHEN ? != '' THEN ? ELSE last_user_agent END,
          os_info = CASE WHEN ? != '' THEN ? ELSE os_info END,
          browser_info = CASE WHEN ? != '' THEN ? ELSE browser_info END,
          device_info = CASE WHEN ? != '' THEN ? ELSE device_info END
      WHERE id = ?
    `).run(ip, ip, userAgent, userAgent, os, os, browser, browser, device, device, prof.user_id);
  }
}

/**
 * Инкремент счётчика просмотров профиля.
 * @param {string} slug - Slug профиля
 */
export function incrementViewCount(slug) {
  const db = getDatabase();
  db.prepare('UPDATE profiles SET view_count = view_count + 1 WHERE slug = ?').run(slug);
}

/**
 * Получение списка всех профилей с привязанными e-mail адресами.
 * @returns {object[]} Массив профилей
 */
export function getAllProfiles() {
  const db = getDatabase();
  const now = Date.now();

  // Автоматическое связывание и сидинг профилей для всех существующих учетных записей в users
  try {
    const orphanUsers = db.prepare(`
      SELECT u.id, u.display_name, u.is_admin
      FROM users u
      LEFT JOIN profiles p ON p.user_id = u.id OR p.id = u.id
      WHERE p.id IS NULL
    `).all();

    for (const u of orphanUsers) {
      const profileId = `profile_${u.id}`;
      let baseSlug = (u.display_name || 'user')
        .toLowerCase()
        .trim()
        .replace(/[^a-z0-9_-]/g, '');
      if (!baseSlug || baseSlug.length < 2) {
        baseSlug = `user_${u.id.slice(-6)}`;
      }

      const existingSlug = db.prepare('SELECT id FROM profiles WHERE slug = ?').get(baseSlug);
      if (existingSlug && existingSlug.id !== profileId) {
        baseSlug = `${baseSlug}_${u.id.slice(-4)}`;
      }

      try {
        db.prepare(`
          INSERT INTO profiles (
            id, user_id, display_name, slug, bio_text, is_owner, created_at, updated_at
          ) VALUES (?, ?, ?, ?, 'Зарегистрированный аккаунт Hoshizune', ?, ?, ?)
        `).run(profileId, u.id, u.display_name || 'Пользователь', baseSlug, u.is_admin ? 1 : 0, now, now);
      } catch {}
    }
  } catch (err) {
    console.error('[getAllProfiles] Ошибка автосинхронизации неLinked пользователей:', err);
  }

  // Выборка всех пользователей напрямую из users таблицы
  const rows = db
    .prepare(`
      SELECT 
        COALESCE(p.id, 'profile_' || u.id) as id,
        u.id as user_id,
        COALESCE(p.display_name, u.display_name, 'Пользователь') as display_name,
        COALESCE(p.slug, 'user_' || SUBSTR(u.id, -6)) as slug,
        COALESCE(p.bio_text, 'Зарегистрированный аккаунт Hoshizune') as bio_text,
        COALESCE(p.avatar_path, u.avatar_url, '') as avatar_path,
        COALESCE(p.accent_color, '#ffffff') as accent_color,
        COALESCE(p.status, 'offline') as status,
        COALESCE(p.level, u.level, 1) as level,
        COALESCE(p.view_count, 0) as view_count,
        COALESCE(p.is_owner, CASE WHEN u.role = 'owner' THEN 1 ELSE 0 END) as is_owner,
        COALESCE(p.created_at, u.created_at) as created_at,
        COALESCE(p.updated_at, u.updated_at) as updated_at,
        CASE WHEN p.is_owner = 1 THEN 'owner' WHEN u.is_admin = 1 THEN 'admin' ELSE 'user' END as user_role,
        COALESCE(ue1.email, ue2.email, ue3.email) as user_email
      FROM users u
      LEFT JOIN profiles p ON p.user_id = u.id OR p.id = u.id
      LEFT JOIN user_emails ue1 ON ue1.user_id = u.id AND ue1.is_primary = 1
      LEFT JOIN user_emails ue2 ON ue2.user_id = p.user_id AND ue2.is_primary = 1
      LEFT JOIN user_emails ue3 ON ue3.user_id = p.id
      ORDER BY u.created_at DESC
    `)
    .all();

  return rows.map((row) => formatProfile(row));
}

/**
 * Получение списка всех профилей для администраторов с e-mail адресами.
 * Вызывается ТОЛЬКО из защищенных админ-эндпоинтов после проверки isAdminAuthorized().
 * @returns {object[]}
 */
export function getAllProfilesAdmin() {
  const db = getDatabase();
  const rows = db
    .prepare(`
      SELECT 
        COALESCE(p.id, 'profile_' || u.id) as id,
        u.id as user_id,
        COALESCE(p.display_name, u.display_name, 'Пользователь') as display_name,
        COALESCE(p.slug, 'user_' || SUBSTR(u.id, -6)) as slug,
        COALESCE(p.bio_text, 'Зарегистрированный аккаунт Hoshizune') as bio_text,
        COALESCE(p.avatar_path, u.avatar_url, '') as avatar_path,
        COALESCE(p.accent_color, '#ffffff') as accent_color,
        COALESCE(p.status, 'offline') as status,
        COALESCE(p.level, u.level, 1) as level,
        COALESCE(p.view_count, 0) as view_count,
        COALESCE(p.is_owner, CASE WHEN u.role = 'owner' THEN 1 ELSE 0 END) as is_owner,
        COALESCE(p.created_at, u.created_at) as created_at,
        COALESCE(p.updated_at, u.updated_at) as updated_at,
        CASE WHEN p.is_owner = 1 THEN 'owner' WHEN u.is_admin = 1 THEN 'admin' ELSE 'user' END as user_role,
        COALESCE(ue1.email, ue2.email, ue3.email) as user_email
      FROM users u
      LEFT JOIN profiles p ON p.user_id = u.id OR p.id = u.id
      LEFT JOIN user_emails ue1 ON ue1.user_id = u.id AND ue1.is_primary = 1
      LEFT JOIN user_emails ue2 ON ue2.user_id = p.user_id AND ue2.is_primary = 1
      LEFT JOIN user_emails ue3 ON ue3.user_id = p.id
      ORDER BY u.created_at DESC
    `)
    .all();

  return rows.map((row) => {
    const formatted = formatProfile(row);
    return {
      ...formatted,
      userEmail: row.user_email || '',
    };
  });
}

/**
 * Добавление комментария к профилю.
 * @param {object} data - Данные комментария
 * @returns {object} Созданный комментарий
 */
export function addComment(data) {
  const db = getDatabase();
  const now = Date.now();

  const result = db.prepare(`
    INSERT INTO comments (
      profile_id, author_id, author_name,
      text, rating, created_at
    ) VALUES (?, ?, ?, ?, ?, ?)
  `).run(
    data.profileId,
    data.authorId,
    data.authorName || 'Аноним',
    data.text,
    data.rating || 0,
    now
  );

  return {
    id: result.lastInsertRowid,
    profileId: data.profileId,
    authorId: data.authorId,
    authorName: data.authorName || 'Аноним',
    text: data.text,
    rating: data.rating || 0,
    createdAt: now,
  };
}

/**
 * Получение всех комментариев профиля.
 * @param {string} profileId - ID профиля
 * @returns {object[]} Массив комментариев
 */
export function getCommentsByProfileId(profileId) {
  const db = getDatabase();
  const rows = db
    .prepare('SELECT * FROM comments WHERE profile_id = ? ORDER BY created_at DESC')
    .all(profileId);

  return rows.map((r) => ({
    id: r.id,
    profileId: r.profile_id,
    authorId: r.author_id,
    authorName: r.author_name,
    text: r.text,
    rating: r.rating,
    createdAt: r.created_at,
  }));
}

/**
 * Псевдоним getComments для обратной совместимости.
 */
export const getComments = getCommentsByProfileId;

/**
 * Вспомогательное форматирование строки базы данных в объект профиля.
 * @param {object} row - Строка из таблицы profiles
 * @returns {object} Сформатированный профиль
 */
function formatProfile(row) {
  const db = getDatabase();
  const now = Date.now();
  const isRecentlyOnline = now - row.last_seen < HEARTBEAT_TIMEOUT_MS;

  let effectiveStatus = row.status;
  if (row.status === USER_STATUSES.INVISIBLE) {
    effectiveStatus = 'offline';
  } else if (!isRecentlyOnline && row.status !== USER_STATUSES.DND) {
    effectiveStatus = 'offline';
  }

  let links = [];
  try {
    links = JSON.parse(row.links || '[]');
  } catch {
    links = [];
  }

  // Запрашиваем актуальные роли и титулы из таблицы users
  let userRow = null;
  const targetUserId = row.user_id || row.id;
  let userEmail = '';
  let accountToken = null;
  if (targetUserId) {
    try {
      userRow = db.prepare('SELECT role, title_id, roles_json, titles_json, level, account_token FROM users WHERE id = ?').get(targetUserId);
      accountToken = userRow?.account_token || null;
      const emailRow = db.prepare('SELECT email FROM user_emails WHERE user_id = ? ORDER BY is_primary DESC').get(targetUserId);
      if (emailRow) userEmail = emailRow.email;
    } catch {}
  }

  let userRoles = [];
  try {
    userRoles = JSON.parse(userRow?.roles_json || '[]');
  } catch {
    userRoles = [];
  }
  if (!Array.isArray(userRoles) || userRoles.length === 0) {
    userRoles = [userRow?.role || (row.is_owner ? 'owner' : 'user')];
  }

  // Неприкосновенность роли Owner: аккаунт владельца всегда сохраняет роль 'owner' на первом месте
  if (row.is_owner === 1 || userRow?.role === 'owner' || userRow?.display_name?.toLowerCase() === 'zerosense' || row.slug?.toLowerCase() === 'hoshizune') {
    if (!userRoles.includes('owner')) {
      userRoles.unshift('owner');
    }
  }

  userRoles = sortRolesByHierarchy(userRoles);

  let userTitles = [];
  try {
    userTitles = JSON.parse(userRow?.titles_json || '[]');
  } catch {
    userTitles = [];
  }
  if (!Array.isArray(userTitles) || userTitles.length === 0) {
    const t = userRow?.title_id || row.title_id || row.custom_title;
    if (t) userTitles = [t];
  }

  const primaryRole = getPrimaryRole(userRoles);
  const primaryTitle = userTitles[0] || null;

  // Построение ролевых и косметических бейджей по иерархии
  const badges = [];
  const addedBadgeIds = new Set();

  // 0. Активная подписка
  let sub = null;
  if (targetUserId) {
    try {
      sub = getUserActiveSubscription(targetUserId);
    } catch {}
  }
  const hasSubscription = !!sub || row.is_owner === 1;

  if (sub) {
    badges.push({
      id: 'sub_vip',
      name: `Подписка ${sub.planName}`,
      badgeText: `💎 ${sub.planName.toUpperCase()}`,
      color: '#c084fc',
      type: 'subscription',
      isSystem: false,
    });
    addedBadgeIds.add('sub_vip');
  }

  // 1. Системные роли (Owner, Admin, Support)
  for (const rId of userRoles) {
    if (rId && rId !== 'user' && !addedBadgeIds.has(rId)) {
      const sysRole = getRoleById(rId);
      if (sysRole) {
        badges.push({
          id: sysRole.id,
          name: sysRole.name,
          badgeText: sysRole.badgeText,
          color: sysRole.color,
          type: 'role',
          isSystem: true,
        });
        addedBadgeIds.add(sysRole.id);
      }
    }
  }

  // 2. Косметические титулы (VIP, Вайбкодер и т.д.)
  for (const tId of userTitles) {
    if (tId && !addedBadgeIds.has(tId)) {
      const titleRole = getRoleById(tId);
      if (titleRole) {
        badges.push({
          id: titleRole.id,
          name: titleRole.name,
          badgeText: titleRole.badgeText,
          color: titleRole.color,
          type: 'title',
          isSystem: false,
        });
        addedBadgeIds.add(titleRole.id);
      }
    }
  }

  return {
    id: row.id,
    slug: row.slug,
    displayName: row.display_name,
    bioText: row.bio_text,
    avatarPath: row.avatar_path,
    accentColor: row.accent_color,
    links: sanitizeLinks(links),
    status: row.status,
    effectiveStatus,
    lastSeen: row.last_seen,
    viewCount: row.view_count,
    level: userRow?.level || row.level || 1,
    isOwner: !!row.is_owner || userRoles.includes('owner'),
    role: primaryRole,
    titleId: primaryTitle,
    roles: userRoles,
    titles: userTitles,
    badges,
    allowComments: row.allow_comments !== undefined ? !!row.allow_comments : true,
    restrictions: (() => {
      try {
        const raw = row.restrictions || userRow?.restrictions || '[]';
        return JSON.parse(raw);
      } catch {
        return [];
      }
    })(),
    userId: targetUserId,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
}

/**
 * Валидация и санитизация ссылок в профиле.
 * Отклоняет javascript:, data:, file: и любые не-http/https протоколы.
 * @param {Array} links
 * @returns {Array}
 */
export function sanitizeLinks(links) {
  if (!Array.isArray(links)) return [];
  return links
    .filter((l) => {
      if (!l || typeof l !== 'object') return false;
      const url = String(l.url || '').trim();
      if (!url) return false;
      try {
        const parsed = new URL(url);
        return parsed.protocol === 'http:' || parsed.protocol === 'https:';
      } catch {
        return false;
      }
    })
    .map((l) => ({
      ...l,
      label: typeof l.label === 'string' ? l.label.slice(0, 100) : '',
      url: String(l.url || '').trim().slice(0, 2048),
    }));
}

/**
 * Преобразование профиля в безопасный публичный объект (Allowlist полей).
 * Гарантирует отсутствие PII (userEmail) и секретов (accountToken).
 * @param {object|null} profile
 * @returns {object|null}
 */
export function toPublicProfile(profile) {
  if (!profile) return null;
  return {
    id: profile.id,
    slug: profile.slug,
    displayName: profile.displayName,
    bioText: profile.bioText || '',
    avatarPath: profile.avatarPath || null,
    accentColor: profile.accentColor || '#ffffff',
    links: sanitizeLinks(profile.links || []),
    status: profile.status || 'offline',
    effectiveStatus: profile.effectiveStatus || 'offline',
    lastSeen: profile.lastSeen || 0,
    viewCount: profile.viewCount || 0,
    level: profile.level || 1,
    isOwner: !!profile.isOwner,
    role: profile.role || 'user',
    titleId: profile.titleId || null,
    roles: profile.roles || [],
    titles: profile.titles || [],
    badges: profile.badges || [],
    allowComments: profile.allowComments !== false,
    restrictions: profile.restrictions || [],
    createdAt: profile.createdAt || 0,
    updatedAt: profile.updatedAt || 0,
  };
}

/**
 * Редактирование комментария администратором.
 */
export function updateComment(id, text, rating) {
  const db = getDatabase();
  db.prepare('UPDATE comments SET text = ?, rating = ? WHERE id = ?').run(text, rating, id);
}

/**
 * Удаление комментария администратором.
 */
export function deleteCommentAdmin(id) {
  const db = getDatabase();
  db.prepare('DELETE FROM comments WHERE id = ?').run(id);
}

