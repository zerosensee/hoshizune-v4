...
export function getAllProfiles() {
  ...
  try {
    const orphanUsers = db.prepare(`
      SELECT u.id, u.display_name, u.is_admin, u.role
      FROM users u
      LEFT JOIN profiles p ON p.user_id = u.id OR LOWER(p.display_name) = LOWER(u.display_name) OR LOWER(p.slug) = LOWER(u.display_name)
      WHERE p.id IS NULL
    `).all();
