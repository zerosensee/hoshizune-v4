...
export function updateProfile(id, data) {
  ...
  if (data.level !== undefined) {
    const lvl = parseInt(data.level, 10) || 1;
    fields.push('level = ?');
    values.push(lvl);
  ...
  if (data.viewCount !== undefined) {
    ...
    values.push(parseInt(data.viewCount, 10) || 0);
  }
...
function formatProfile(row) {
  ...
  if (targetUserId) {
    try {
      userRow = db.prepare('SELECT role, title_id, roles_json, titles_json, level FROM users WHERE id = ?').get(targetUserId);
    } catch {}
  }
