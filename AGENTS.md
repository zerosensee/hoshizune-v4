...
1. In EVERY response where code changes or deployment are discussed, ALWAYS provide:
   ...
   - Commands for VPS (single-line command without `cd` since the user is already inside the repo).
2. NEVER add `cd /var/www/...` to VPS commands. The user is already in the repository directory on VPS.
3. Commit messages MUST be specific and detailed, explicitly listing what was fixed. Never use generic messages like "update".
