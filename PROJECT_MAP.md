...
16. **[Feature & Diagnostics] Внедрение Debug HUD и исправление тем оформления**:
    - Создан эндпоинт `/api/debug` (хеш коммита, uptime, целостность БД, количество записей).
    - Разработан компонент `components/ui/DebugPanel.jsx` с перехватом ошибок консоли, статуса сети, текущей темы и кнопкой экспорта отчёта в буфер обмена в 1 клик.
    - В `app/layout.jsx` расширен inline-скрипт инициализации для мгновенной загрузки всех 12+ пресетов тем без мигания.
    - В `app/settings/page.jsx` возвращён гостевой доступ к настройкам темы и шрифтов без принудительного редиректа на `/auth`.
    - База данных `data/` снята с трекинга git (`git rm --cached`), устранив конфликты при `git pull` на боевом сервере.
